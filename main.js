...
app.get('/', function(req, res){
    db.query(`select num, title, author, date_format(created, '%Y-%m-%d %H:%i:%s') as created, count from board order by num desc`, function(error, boards){
    var title ='게시판입니다.'
    var list = '<tr>';
...
app.get('/page/:pageId', function(req, res){
    db.query(`select * count from board`, function(error, boards){
        var pageId = req.params.pageId;
        db.query('update board set count = count +1 where num = ?', [pageId], function(error2, board3){
        });
        db.query(`select num, title, author, description, date_format(created, '%Y-%m-%d %H:%i:%s') as created, count from board where num=?`, [pageId], function(error2, board){
